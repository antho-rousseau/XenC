 ...
 *
 * $Id: biXEntropy.cpp, v 1.0 PUBLIC RELEASE 2013/07/16 rousseau Exp $
 */
...
int BiXEntropy::launch() {
    ...
    if (opt->getOutTLM()->getFileName().compare("") == 0) {
        Mode::findSampleSize(sD->getTargetCorps()->getPtrInCorp(), sD->getTargetCorps()->getPtrOutCorp());
        
        shared_ptr<Corpus> ptrOutLMCorp(new Corpus(Mode::extractSample(sD->getTargetCorps()->getPtrOutCorp(), opt->getSampleSize(), opt->getMean())));
    ...
    if (!exists(sD->getSourceLMs()->getPtrInLM()->getFileName())) { cout << "Error: LM file " + sD->getSourceLMs()->getPtrInLM()->getFileName() + " does not exists!" << endl; return 1; }
    if (!exists(sD->getTargetLMs()->getPtrInLM()->getFileName())) { cout << "Error: LM file " + sD->getTargetLMs()->getPtrInLM()->getFileName() + " does not exists!" << endl; return 1; }
    if (!exists(sD->getSourceLMs()->getPtrOutLM()->getFileName())) { cout << "Error: LM file " + sD->getSourceLMs()->getPtrOutLM()->getFileName() + " does not exists!" << endl; return 1; }
    if (!exists(sD->getTargetLMs()->getPtrOutLM()->getFileName())) { cout << "Error: LM file " + sD->getTargetLMs()->getPtrOutLM()->getFileName() + " does not exists!" << endl; return 1; }

    // Init Mean LMs if needed
    if (opt->getMean()) {
        shared_ptr<Corpus> ptrSourceOutLMCorp2(new Corpus(Mode::extractSample(sD->getSourceCorps()->getPtrOutCorp(), opt->getSampleSize(), opt->getMean())));
        ...
        shared_ptr<Corpus> ptrTargetOutLMCorp2(new Corpus(Mode::extractSample(sD->getTargetCorps()->getPtrOutCorp(), opt->getSampleSize(), opt->getMean())));
        ...
        shared_ptr<Corpus> ptrSourceOutLMCorp3(new Corpus(Mode::extractSample(sD->getSourceCorps()->getPtrOutCorp(), opt->getSampleSize(), opt->getMean())));
        ...
        shared_ptr<Corpus> ptrTargetOutLMCorp3(new Corpus(Mode::extractSample(sD->getTargetCorps()->getPtrOutCorp(), opt->getSampleSize(), opt->getMean())));
        ...
        if (!exists(sD->getMeanSourceLMs()->getPtrOutLM2()->getFileName())) { cout << "Error: LM file " + sD->getMeanSourceLMs()->getPtrOutLM2()->getFileName() + " does not exists!" << endl; return 1; }
        if (!exists(sD->getMeanTargetLMs()->getPtrOutLM2()->getFileName())) { cout << "Error: LM file " + sD->getMeanTargetLMs()->getPtrOutLM2()->getFileName() + " does not exists!" << endl; return 1; }
        if (!exists(sD->getMeanSourceLMs()->getPtrOutLM3()->getFileName())) { cout << "Error: LM file " + sD->getMeanSourceLMs()->getPtrOutLM3()->getFileName() + " does not exists!" << endl; return 1; }
        if (!exists(sD->getMeanTargetLMs()->getPtrOutLM3()->getFileName())) { cout << "Error: LM file " + sD->getMeanTargetLMs()->getPtrOutLM3()->getFileName() + " does not exists!" << endl; return 1; }
    }
    ...
    if (opt->getStem()) {
        ...
        Mode::findSampleSize(sD->getStemTargetCorps()->getPtrInCorp(), sD->getStemTargetCorps()->getPtrOutCorp());
        shared_ptr<Corpus> ptrOutTargetStemCorp(new Corpus(Mode::extractSample(sD->getStemTargetCorps()->getPtrOutCorp(), opt->getSampleSize(), false)));
        ...
        if (!exists(sD->getStemTargetLMs()->getPtrInLM()->getFileName())) { cout << "Error: LM file " + sD->getStemTargetLMs()->getPtrInLM()->getFileName() + " does not exists!" << endl; return 1; }
        ...
        if (!exists(sD->getStemTargetLMs()->getPtrOutLM()->getFileName())) { cout << "Error: LM file " + sD->getStemTargetLMs()->getPtrOutLM()->getFileName() + " does not exists!" << endl; return 1; }
    }
    ...
    // Result writing
    cout << "NB Scores: " + toString(sD->getScHold()->getPtrScores()->getSize()) + " NB Source corp (unclean): " + toString(sD->getSourceCorps()->getPtrOutCorp()->getSize()) + " NB Target corp (unclean): " + toString(sD->getTargetCorps()->getPtrOutCorp()->getSize()) << endl;
    XenIO::cleanCorpusBi(sD->getSourceCorps()->getPtrOutCorp(), sD->getTargetCorps()->getPtrOutCorp(), sD->getScHold()->getPtrScores());
    cout << "NB Scores: " + toString(sD->getScHold()->getPtrScores()->getSize()) + " NB Source corp (clean): " + toString(sD->getSourceCorps()->getPtrOutCorp()->getSize()) + " NB Target corp (clean): " + toString(sD->getTargetCorps()->getPtrOutCorp()->getSize()) << endl;
